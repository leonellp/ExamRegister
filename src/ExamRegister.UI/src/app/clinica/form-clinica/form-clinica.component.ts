...
export class FormClinicaComponent extends BaseFormComponent implements OnInit {
  ...
  semFoco() {
    let idestado = Object.assign({}, this.formulario.value).endereco.estado
      .idestado;
    if (idestado) {
  ...
  procuraEstado: OperatorFunction<
    string,
    readonly { idestado: string; nome: string }[]
  > = (text$: Observable<string>) =>
    text$.pipe(
      ...
      map((term) =>
        this.estados
          .filter((estado) => new RegExp(term, 'mi').test(estado.nome))
          .slice(0, 10)
      )
    );

  procuraCidade: OperatorFunction<
    string,
    readonly { idcidade: string; nome: string }[]
  > = (text$: Observable<string>) =>
    text$.pipe(
      debounceTime(200),
      distinctUntilChanged(),
      ...
      map((term) =>
        this.cidades
          .filter((cidade) => new RegExp(term, 'mi').test(cidade.nome))
          .slice(0, 10)
      )
    );
  ...
  submit() {
    ...
    if (valueSubmit.endereco.estado)
      valueSubmit.endereco.idestado = valueSubmit.endereco.estado.idestado;

    if (valueSubmit.endereco.idestado)
      valueSubmit.endereco.idcidade = valueSubmit.endereco.cidade.idcidade;
